...
/// Removes spaces immediately before commas.
...
test "space before comma removes spaces before comma" {
...
test "space before comma keeps already formatted commas" {
    const allocator = std.testing.allocator;
    const input = "call(a, b, c)\n";
    var result = try apply(allocator, input);
    defer result.deinit(allocator);
...
test "space before comma ignores strings and comments" {
    const allocator = std.testing.allocator;
