...
/// Removes spaces immediately inside reference brackets, e.g. `items[ 0 ]`.
pub fn apply(allocator: std.mem.Allocator, source: []const u8) !Result {
    ...
    while (i < source.len) {
        ...
        if (ch == '[' and isReferenceStart(builder.items)) {
        ...
        if (ch == ']' and isReferenceEnd(builder.items)) {
...
fn isReferenceStart(builder_items: []u8) bool {
...
fn isReferenceEnd(builder_items: []u8) bool {
    const prev = utils.findPrevNonWhitespace(builder_items) orelse return false;
...
test "space inside reference brackets removes inner spaces" {
    ...
    try std.testing.expectEqualStrings("value = items[0]\n", result.buffer);
}

test "space inside reference brackets leaves array literal unchanged" {
    const allocator = std.testing.allocator;
...
test "space inside reference brackets ignores strings and comments" {
    const allocator = std.testing.allocator;
    const input = "puts \"items[ 0 ]\"\n# items[ 0 ]\nvalue = items[ 0 ]\n";
    var result = try apply(allocator, input);
    defer result.deinit(allocator);

    try std.testing.expect(result.changed);
    try std.testing.expectEqualStrings("puts \"items[ 0 ]\"\n# items[ 0 ]\nvalue = items[0]\n", result.buffer);
}
